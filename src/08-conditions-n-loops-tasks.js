...
function getFactorial(n) {
  // throw new Error('Not implemented');
  return [...new Array(n).fill(1)].map((el, i) => el + i).reduce((acc, curr) => acc * curr, 1);
}
...
function getSumBetweenNumbers(n1, n2) {
  // throw new Error('Not implemented');
  const len = n2 - n1 + 1;
  return [...new Array(len).fill(n1)].map((el, i) => el + i).reduce((acc, curr) => acc + curr, 0);
}
...
function isInsideCircle(circle, point) {
  // throw new Error('Not implemented');
  // const hipo = Math.sqrt((point.x ** point.x) + (point.y ** point.y));
  // const rad = Math.sqrt(circle.radius ** circle.radius);
  const x = (point.x - circle.center.x) * (point.x - circle.center.x);
  const y = (point.y - circle.center.y) * (point.y - circle.center.y);
  if (x + y < circle.radius * circle.radius) {
    return true;
  }
  return false;
}
...
function findFirstSingleChar(str) {
  // throw new Error('Not implemented');
  return str.split('').filter((c) => !(str.split(c).length - 2))[0] || null;
}
 ...
 */
function getCommonDirectoryPath(/* spathes */) {
  throw new Error('Not implemented');
}
