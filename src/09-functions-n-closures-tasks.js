...
//   // throw new Error('Not implemented');
//   const att = new Array(attempts - 1).fill(null);
//   return () => {
//     att.fill(null).forEach(() => {
//       try {
//         return func();
//       } catch (error) {
//         const tt = attempts - 1;
//         return retry(func, tt)();
//       }
//     });
//     return undefined;
//   };
...
function logger(func, logFunc) {
  // throw new Error('Not implemented');
  return (...x) => {
    const arg = `${func.name}(${JSON.stringify(x).slice(1, -1)}) starts`;
    const arg1 = `${func.name}(${JSON.stringify(x).slice(1, -1)}) ends`;
    logFunc(arg);
    const returnFunc = func(...x);
    logFunc(arg1);
    return returnFunc;
  };
}
